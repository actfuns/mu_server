				...
				if (CS$<>8__locals1.curEraID > 0)
				{
					...
					if (CurrentDayID != this.CheckRankAwardDayID && this.InRankAwardTime())
					{
						...
						if (null != kfEraRankData)
						{
							...
							if (CS$<>8__locals1.curEraID != RankAwardEraID)
							{
								...
								using (List<JunTuanBaseData>.Enumerator enumerator = juntuanBaseList.GetEnumerator())
								{
									while (enumerator.MoveNext())
									{
										...
										{
											KFEraData kfEraData = JunTuanClient.getInstance().GetJunTuanEraData(CS$<>8__locals2.baseData.JunTuanId, false);
											if (null == kfEraData)
											{
												LogManager.WriteLog(LogTypes.Error, string.Format("纪元{0}奖励 军团{1} 获取纪元数据失败", CS$<>8__locals1.curEraID, CS$<>8__locals2.baseData.JunTuanId), null, true);
											}
											else
											{
												int RankNum = kfEraRankData.FindIndex((KFEraRankData x) => x.JunTuanID == CS$<>8__locals2.baseData.JunTuanId);
												...
												EraAwardConfig awardConfig = AwardsConfigList.Find((EraAwardConfig x) => x.AwardType == 2 && x.EraID == CS$<>8__locals1.curEraID && x.EraRanking == RankNum);
												if (null == awardConfig)
												{
													LogManager.WriteLog(LogTypes.Error, string.Format("纪元{0}奖励 军团{1} 排行{2} 获取奖励配置失败", CS$<>8__locals1.curEraID, CS$<>8__locals2.baseData.JunTuanId, RankNum), null, true);
												}
												else
												{
													if (awardConfig.Progress == 4)
													{
														...
														{
															LogManager.WriteLog(LogTypes.Error, string.Format("纪元{0}奖励 军团{1} 排行{2} 进度不满足要求", CS$<>8__locals1.curEraID, CS$<>8__locals2.baseData.JunTuanId, RankNum), null, true);
															continue;
														}
													}
													...
													{
														LogManager.WriteLog(LogTypes.Error, string.Format("纪元{0}奖励 军团{1} 排行{2} 获取juntuanRoleList失败", CS$<>8__locals1.curEraID, CS$<>8__locals2.baseData.JunTuanId, RankNum), null, true);
													}
													else
													{
														foreach (int bhid in CS$<>8__locals2.baseData.BhList)
														{
