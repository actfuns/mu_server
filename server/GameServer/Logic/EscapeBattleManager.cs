...
namespace GameServer.Logic
{
	...
	public class EscapeBattleManager : ICopySceneManager, IManager, ICmdProcessorEx, ICmdProcessor, IManager2, IEventListener, IEventListenerEx
	{
		...
		public void PushGameResultData(EscapeBattleScene scene, int zhanduiId)
		{
			...
			List<EscapeBattleRoleInfo> clientContextDataList;
			if (CS$<>8__locals1.scene.ClientContextDataDict.TryGetValue(zhanduiId, out clientContextDataList))
			{
				foreach (EscapeBattleRoleInfo contextData in clientContextDataList)
				{
					GameClient client = null;
					if (CS$<>8__locals1.scene.ClientDict.TryGetValue(contextData.RoleID, out client))
					{
						...
						if (null != roleAnalysisData)
						{
							...
							if (contextData.ZhanDuiID == CS$<>8__locals1.scene.GameStatisticalData.ZhanDuiIDWin)
							{
			...
			if (CS$<>8__locals1.scene.GameStatisticalData.ZhanDuiDict.TryGetValue(zhanduiId, out zhanduiData))
			{
				...
				{
					EscapeBattleTeamInfo teamInfo = CS$<>8__locals1.scene.ScoreData.BattleTeamList.Find((EscapeBattleTeamInfo x) => x.TeamID == zhanduiData.ZhanDuiID);
					if (null != teamInfo)
					{
						...
						if (null != awardConfig)
						{
							...
							{
								List<int> AliveVsJiFen = awardConfig.WinRankValue.Find((List<int> x) => x[0] == CS$<>8__locals1.scene.GameStatisticalData.WinZhanDuiAliveCount);
							...
							{
								int leaveNum = CS$<>8__locals1.scene.ScoreData.BattleTeamList.Count - teamInfo.RankNum + 1;
			...
			KeyValuePair<int, List<int>> resultPair = new KeyValuePair<int, List<int>>(CS$<>8__locals1.scene.GameId, ZhanDuiIDVsModJiFen);
