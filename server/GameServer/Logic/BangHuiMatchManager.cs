					...
					if (null != CurRoundPKInfo)
					{
						for (int i = 0; i < CurRoundPKInfo.Count; i++)
						{
							...
							if (null != bhData)
							{
								...
								pkInfo.winpct1 = ((goldRound == 1) ? 0 : ((byte)((double)bhData.cur_win / (double)(goldRound - 1) * 100.0)));
								pkInfo.rank1 = (byte)(rankInfoList.FindIndex((BangHuiMatchRankInfo x) => x.Key == pkInfo.bhid1) + 1);
							}
							if (null != bhData2)
							{
								pkInfo.win2 = (byte)bhData2.cur_win;
								pkInfo.winpct2 = ((goldRound == 1) ? 0 : ((byte)((double)bhData2.cur_win / (double)(goldRound - 1) * 100.0)));
